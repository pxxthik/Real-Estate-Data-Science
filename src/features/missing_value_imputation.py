...
def impute_builtUpArea(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        all_present_df = df.dropna(
            subset=["super_built_up_area", "built_up_area", "carpet_area"]
        )

        super_to_built_up_ratio = (
            all_present_df["super_built_up_area"] / all_present_df["built_up_area"]
        ).median()
        carpet_to_built_up_ratio = (
            all_present_df["carpet_area"] / all_present_df["built_up_area"]
        ).median()
        ...
        df.loc[sbc_mask, "built_up_area"] = (
            (
                ...
                + (df.loc[sbc_mask, "carpet_area"] / carpet_to_built_up_ratio)
            )
            / 2
        ).round()
        ...
        df.loc[sb_mask, "built_up_area"] = (
            df.loc[sb_mask, "super_built_up_area"] / super_to_built_up_ratio
        ).round()
        ...
        df.loc[c_mask, "built_up_area"] = (
            df.loc[c_mask, "carpet_area"] / carpet_to_built_up_ratio
        ).round()
...
if __name__ == "__main__":
    try:
        ...
        # Pipeline
        df = df.pipe(impute_builtUpArea).pipe(impute_AgePossion).pipe(impute_floorNum)
